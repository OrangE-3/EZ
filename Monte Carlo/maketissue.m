...
myname      = 'HIE';% name for files: myname_T.bin, myname_H.mci  
time_min    = 5;      	% time duration of the simulation [min] <----- run time -----
nm          = 990;   	% desired wavelength of simulation
Nbins       = 200;    	% # of bins in each dimension of cube 
binsize     = 0.03375; 	% size of each bin, eg. [cm] or [mm]
...
radius      = 1.25;   % 1/e radius of beam at tissue surface
waist       = 1.25;  	% 1/e radius of beam at focus
...
T = T + 7;      % fill background with white matter

zsurf = 0.11;  % position of air/episkin surface

pepi_dermi=0.05+zsurf;
pdermi_font=0.09+pepi_dermi;
pfont_csf=pdermi_font+0.22;
pcsf_gm=pfont_csf+0.24;
pgm_wm=pcsf_gm+1.32;
...
for iz=1:Nz % for every depth z(iz)
    ...
    % epidermis 
    if iz>round(zsurf/dz) & iz<=round((pepi_dermi)/dz)
        T(:,:,iz) = 2; 
    end
    
    % dermis
    if iz>round(pepi_dermi/dz) & iz<=round((pdermi_font)/dz)
        T(:,:,iz) = 3; 
    end
    
    % fontanelle
    if iz>round(pdermi_font/dz) & iz<=round((pfont_csf)/dz)
        T(:,:,iz) = 4; 
    end
    
    % csf
    if iz>round(pfont_csf/dz) & iz<=round((pcsf_gm)/dz)
        T(:,:,iz) = 5; 
    end
    
    % graymatter
    if iz>round(pcsf_gm/dz) & iz<=round((pgm_wm)/dz)
        T(:,:,iz) = 6; 
    end
